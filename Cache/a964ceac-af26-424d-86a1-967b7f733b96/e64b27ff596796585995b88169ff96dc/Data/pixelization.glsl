...
uniform NF_PRECISION                  float  pixelOffset; // Title: Size	
...
#define Node19_Texture_2D_Object_Parameter( Globals ) /*nothing*/
#define Node3_Screen_UV_Coord( ScreenCoord, Globals ) ScreenCoord = Globals.gScreenCoord
#define Node7_Camera_Aspect( Camera_Aspect, Globals ) Camera_Aspect = ngsCameraAspect
void Node5_Float_Parameter( out float Output, ssGlobals Globals ) { Output = pixelOffset; }
#define Node10_Multiply( Input0, Input1, Output, Globals ) Output = Input0 * Input1
#define Node8_Construct_Vector( Value1, Value2, Value, Globals ) Value.x = Value1; Value.y = Value2
#define Node0_Pixelize_Coords( Input0, Input1, Output, Globals ) Output = floor( Input0 * Input1 ) / Input1
#define Node1_Texture_2D_Sample( UVCoord, Color, Globals ) Color = SC_SAMPLE_TEX_R(screenTexture, UVCoord, 0.0)
//-----------------------------------------------------------------------------
...
void main() 
{
	...
	{
		Node19_Texture_2D_Object_Parameter( Globals );
		float2 ScreenCoord_N3 = float2(0.0); Node3_Screen_UV_Coord( ScreenCoord_N3, Globals );
		float Camera_Aspect_N7 = 0.0; Node7_Camera_Aspect( Camera_Aspect_N7, Globals );
		float Output_N5 = 0.0; Node5_Float_Parameter( Output_N5, Globals );
		float Output_N10 = 0.0; Node10_Multiply( Camera_Aspect_N7, Output_N5, Output_N10, Globals );
		float2 Value_N8 = float2(0.0); Node8_Construct_Vector( Output_N10, Output_N5, Value_N8, Globals );
		float2 Output_N0 = float2(0.0); Node0_Pixelize_Coords( ScreenCoord_N3, Value_N8, Output_N0, Globals );
		float4 Color_N1 = float4(0.0); Node1_Texture_2D_Sample( Output_N0, Color_N1, Globals );
